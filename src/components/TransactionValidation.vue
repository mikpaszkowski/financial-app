    ...
    <div class="val-background" v-if="modalOn">
      <div id="val-window" v-if="modalShown">
        <div id="errors">
          ...
          <ul>
            <li v-for="error in occuredErrors" :key="error">{{ error }}</li>
          </ul>
        </div>
        <button id="button-ok" @click="clickedOk">OK</button>
      </div>
    </div>
  </transition>
</template>
...
.validation-leave-active {
  transition: all 0.4s;
  animation: 0.2s both sizing reverse;
}

.validation-enter-active {
  transition: all 0.4s;
  animation: 0.2s both sizing;
}
...
.val-background {
  ...
  #button-ok {
    ...
    width: 10rem;
    height: 3rem;
    background-color: rgb(146, 255, 173);
    border-radius: 10rem;
    margin-top: 1.25rem;
